...
export class FlowSpotifyApi {
  ...
  async getShowsByIds(showIds: string[]): Promise<SpotifyApi.ShowObjectSimplified[]> {
  ...
  async getEpisodesUrisFromShowsAfterDate(showIds: string[], dateLimit: Date): Promise<string[]> {
    const showEpisodesResponse = await Promise.all(showIds.map((showId) => this.client.getShowEpisodes(showId)));
    const episodes = showEpisodesResponse.reduce(
      (acc: SpotifyApi.EpisodeObjectSimplified[], response) => [...acc, ...response.body.items],
      [],
    );

    return episodes.reduce((acc: string[], episode: SpotifyApi.EpisodeObjectSimplified) => {
